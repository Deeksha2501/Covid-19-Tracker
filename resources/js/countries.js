...
function createCountryList() {
  const num_coun = country_list.length;
  let i = 0;
  country_list.forEach((country, index) => {
    if (index % Math.ceil(num_coun / num_ul_list) == 0) {
      ul_list_id = `list-${i}`;
    ...
    document.getElementById(
      ul_list_id
    ).innerHTML += `<li id="${country.name}" onClick="fetchData('${country.name}')">${country.name}</li>`;
  });
}
